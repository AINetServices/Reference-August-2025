...
export function Dashboard() {
  ...
  const fetchApplications = async () => {
    try {
      ...
      // DEBUG: Detailed logging for each application
      data?.forEach((app, index) => {
        console.log(`📄 App ${index + 1}:`, {
          ...
          has_extracted_data: !!app.extracted_data,
          extracted_data_keys: app.extracted_data ? Object.keys(app.extracted_data) : 'none'
        })
        
        // Log full extracted_data for the most recent app
        if (index === 0 && app.extracted_data) {
          console.log('🔍 Latest app extracted_data:', JSON.stringify(app.extracted_data, null, 2))
        }
      })
      
      setApplications(data || [])
      
      // Calculate stats - FIXED: Handle all possible statuses
      const newStats = (data || []).reduce((acc, app) => {
        ...
        sent: 0,
        extracted: 0
      } as any) // Use any to allow dynamic keys
